...
##-----------------------------------------------------------------------
## Menu screen for [TimeAttackMode] gamemode
## Allows to select between several time limits and rulesets and compete with
## others in online score ranking
##-----------------------------------------------------------------------

const RANKING_BAR : PackedScene = preload("res://menu/objects/ranking_bar.tscn") ## Ranking bar instance

var selected_time : int = 60 ## Currently selected time limit
var selected_ruleset : int = TimeAttackMode.TIME_ATTACK_RULESET.STANDARD ## Currently selected time attack ruleset
var selected_ranking : String = "total" ## Currently selected score ranking tab

var is_displaying_ranking : bool = false ## Is currently updating score ranking display
...
func _ready() -> void:
	...
	parent_menu.background._change_gradient_colors(Color("46083e"),Color("291239"),Color("1c0610"),Color("1b1b1b"),Color("0b0205"))
	
	await parent_menu.all_screens_added
	cursor = Vector2i(0,0)
	_move_cursor()
	
	#_display_ranking(selected_ranking) # TODO


## Scrolls score ranking scroll bar
...
## Sets time limit to **'time'**[br]
## If **'skip_stats_load'** is true, doesn't update profile stats and score ranking
func _select_time(time : String, skip_stats_load : bool = false) -> void:
	...
	Player.config.gamemode["ta_time_limit"] = selected_time
	if not skip_stats_load:
		_show_stats()
		_display_ranking(selected_ranking)


## Sets time limit to custom **'time'**[br]
...
func _select_ruleset(value : float, skip_stats_load : bool = false) -> void:
	...
	Player.config.gamemode["ta_ruleset"] = selected_ruleset
	if not skip_stats_load:
		_show_stats()
		_display_ranking(selected_ranking)


## Displays profile stats in currently selected time limit + ruleset combination
func _show_stats() -> void:
	...
	# TODO : Link with ranking manager
	# if Player.savedata.hiscores["time_attack_hiscore"].has(entry_string) and UserData.savedata.hiscores["time_attack_hiscore"][entry_string] > 0:
	# 	$Playerdata/Hiscore.text = str(UserData.savedata.hiscores["time_attack_hiscore"][entry_string])
	# else:
	# 	$Playerdata/Hiscore.text = "---"
...
## Displays score ranking of given **'type'** for currently selected time limit + ruleset combination
func _display_ranking(type : String) -> void:
	...
	# 			if data[0] == UserData.profile_name and data[1] == UserData.savedata.vault_key:
...
## Starts online score loading and displays special animation
...
## Starts [TimeAttackMode] game with selected time limit + ruleset combination
func _start_game() -> void:
	...
	match selected_time:
		60 : sec_sound = parent_menu._sound("announce_60sec",null,false)
		120 : sec_sound = parent_menu._sound("announce_120sec",null,false); sound_delay = 0.8
		180 : sec_sound = parent_menu._sound("announce_180sec",null,false)
		300 : sec_sound = parent_menu._sound("announce_300sec",null,false); sound_delay = 0.95
		600 : sec_sound = parent_menu._sound("announce_600sec",null,false); sound_delay = 0.95
		_: sec_sound = parent_menu._sound("announce_custom_sec",null,false)
	
	var mode_sound : AudioStreamPlayer = null
	match selected_ruleset:
		TimeAttackMode.TIME_ATTACK_RULESET.STANDARD: mode_sound = parent_menu._sound("announce_standard",null,false)
		TimeAttackMode.TIME_ATTACK_RULESET.CLASSIC: mode_sound = parent_menu._sound("announce_classic",null,false)
		TimeAttackMode.TIME_ATTACK_RULESET.ARCADE: mode_sound = parent_menu._sound("announce_arcade",null,false)
		TimeAttackMode.TIME_ATTACK_RULESET.COLOR_3: mode_sound = parent_menu._sound("announce_3color",null,false)
		TimeAttackMode.TIME_ATTACK_RULESET.HARDCORE: mode_sound = parent_menu._sound("announce_expert",null,false)
	...
	gamemode.selected_ruleset = selected_ruleset
	gamemode.random_mixes = Player.config.gamemode["ta_random_mixes"]
	
	# TODO
	#Data.main._start_game(ta_skin_metadata, gamemode)


## Called on menu screen close
