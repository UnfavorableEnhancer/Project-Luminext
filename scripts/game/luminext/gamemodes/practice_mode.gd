...
##-----------------------------------------------------------------------
## Plays thru special tutorial sequence which teaches player with game basics
##-----------------------------------------------------------------------

class_name PracticeMode

## All avaiable tutorial stages
enum TUTORIAL_STAGE {
	NONE,
	INTRO,
	MOVE_PIECE,
...
var practice_ui : UIElement = null ## Tutorial main [UIElement]
var tutorial_stage : int = TUTORIAL_STAGE.NONE ## Current tutorial stage
var tutorial_progress : int = 0 ## Used to count current progress in some task
...
func _load_ui() -> void:
	...
	foreground._change_style(game.skin.skin_data, 0.0)
	foreground.ui_elements["progress"].visible = false
...
func _reset() -> int:
	...
	if game.skin == null:
		var skin_change_result : int = await game._change_skin("", true)
		if skin_change_result > 1:
			error_text = "Failed to change skin"
...
func _tutorial_sequence() -> void:
	...
	await game.reset_ended

	game.skin._start()

	# It's easier to do this thru console in silent mode
	Console._command_input("qappend 00 01 00 01", true)
	game.skin.is_music_looping = false
	...
	game.piece_fall_delay = 1.0
	Console._command_input("qappend 00 01 01 00", true)
	...
	Console._command_input("qappend 00 00 00 00", true)
	Console._command_input("qappend 00 00 00 00", true)
	Console._command_input("qappend 00 00 00 00", true)
	...
	Console._command_input("qappend 00 01 00 01", true)
	Console._command_input("qappend 01 00 00 01", true)
	Console._command_input("qappend 01 01 01 00", true)
	...
	Console._command_input("qappend 10 00 01 01", true)
