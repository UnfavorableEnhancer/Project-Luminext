...
@export var data_name : String = "" ## Data entry name inside [SkinData] this button edits
...
func _ready() -> void:
	...
	text = tr("SE_INSERT") + " " + tr("SE_" + data_name.to_upper()).to_upper()
	

## Removes assigned data
...
func _load_data() -> void:
	if editor.skin_data.stream[data_name] == null:
		...
		text = tr("SE_SOME") + " " + tr("SE_" + data_name.to_upper()).to_upper()


## Calls data file select routine in skin editor and then applies selected data
func _assign_data() -> void:
	var format : int = 0
	match file_dialog_to_open:
		"skn" : format = 5
		"texture" : format = 1
		"sound" : format = 2
		"video" : format = 3
	...
	editor._open_file_dialog(format)
...
func _on_press(event : InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed:
		match event.button_index:
			MOUSE_BUTTON_LEFT: _assign_data()
