...
@export var sound_name : String = "" ## Sound entry name inside [SkinData] this button edits
...
## To make button multi-sound you need to add [SpinBox] node named **'N'**[br]
## And make sure that corresponding sound entry inside [SkinData] is [null] Array
var is_multi_sound_button : bool = false
var current_sound : int = 0 ## If its multi-sound button, this int shows current selection

var has_sound : bool = false ## True if sound entry already has sound assigned
...
func _ready() -> void:
	...
	text = tr("SE_INSERT") + " " + tr("SE_A_" + sound_name.to_upper()).to_upper()
	text = text.to_upper()
	...
	if has_node("Play"): 
		get_node("Play").pressed.connect(_play_sound)


## Removes sound sample from entry
...
## Checks sound existance inside [SkinData] and displays corresponding text
func _load_sound(number : int = 0) -> void:
	...
	if is_multi_sound_button:
		if sounds[sound_name][current_sound] == null:
			...
			text = tr("SE_SOME") + " " + tr("SE_A_" + sound_name.to_upper()).to_upper()
			has_sound = true
...
## Calls sound file select routine in skin editor and then applies selected sound sample
func _assign_sound() -> void:
	var path : String
				
	editor._open_file_dialog(2)
...
func _on_press(event : InputEvent) -> void:
	if event is InputEventMouseButton and event.pressed:
		match event.button_index:
			MOUSE_BUTTON_LEFT: _assign_sound()
			MOUSE_BUTTON_RIGHT: _remove_sound()


## Plays currently assigned sound sample
